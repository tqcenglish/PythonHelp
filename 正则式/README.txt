Python核心编程
	十五章
1.	在 Python专门术语中,有两种主要方法完成模式匹配:搜索(searching)和匹配(matching)。
	搜索,即在字符串任意部分中查找匹配的模式,而匹配是指,判断一个字符串能否从起始处全部或部分的匹配某个
模式。
	搜索通过 search()函数或方法来实现,而匹配是以调用 match()函数或方法实现的
2.	元字符(metacharacters)——特殊字符和符号
	记号					说明					例子
	literal				匹配字符串的值			foo
	...
	*					匹配前面的正则式零或多次	[A-Za-Z0-9]*
	...
	\c					逐一匹配特殊字符			
	\A (\Z)				匹配字符串的起始（结束）
3.	问号有两种含义:1.单独使用时表示匹配出现零次或一次的情况,
				2.紧跟在表示重复的元字符后面时,表示要求搜索引擎匹配的字符串越短越好。例如:(+?)
4.例子
	正则表达式模式					匹配的字符串
